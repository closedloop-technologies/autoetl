...
class JsonSchemaParser(BaseJsonSchemaParser):
    ...
    def parse_obj(self, name: str, obj: JsonSchemaObject) -> None:
        if isinstance(obj, list):  # noqa: WPS223
            self.parse_array(name, obj, [])
        elif hasattr(obj, "allOf"):
            self.parse_all_of(name, obj, [])
        elif hasattr(obj, "oneOf"):
            self.parse_root_type(name, obj, [])
        elif hasattr(obj, "is_object"):
...
def jsonschema_to_pydantic(
    ...
    *,
    ...
    parser.parse_obj("OpenAPISchema", obj=schema)
...
if __name__ == "__main__":
    ...
    with open("autoetl/apis/openapi/openapi_spec_20.json", "r") as fh:
        spec = json.load(fh)
        jsonschema_to_pydantic(spec)
